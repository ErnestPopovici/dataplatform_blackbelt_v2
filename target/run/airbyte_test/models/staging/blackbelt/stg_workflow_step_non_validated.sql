  ...
  create view "prod"."public"."stg_workflow_step_non_validated__dbt_tmp" as (
    SELECT
    users.user_id,
...
FROM "prod"."public"."stg_workflow_step_duration" AS workflow_step
left join "prod"."public"."stg_user_belt" AS user_belt
    ON workflow_step.user_belt_id = user_belt.user_belt_id
left join "prod"."public"."stg_jhi_user" AS users
...
WHERE 
    user_belt.color is NOT NULL AND discipline.activated = true AND user_belt.status != 'VALIDATED' AND workflow_step.status != user_belt.status
  ) ;
